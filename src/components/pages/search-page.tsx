...
export function SearchPage() {
  ...
  return <div className="space-y-6" aria-busy={searching}><PageHeader title={roleCopy.title} description={roleCopy.description}/>{user.role==="ADMIN"&&<div className="panel flex items-start gap-3 border-[var(--primary)] p-4 text-sm"><ShieldCheck className="mt-0.5 shrink-0 text-[var(--primary)]" size={18} aria-hidden="true"/><p><strong>Operational search:</strong> Credentials, tokens, payment proofs, and private files are excluded.</p></div>}
    ...
    {!query.trim()&&<section className="grid gap-4 lg:grid-cols-2"><div className="panel p-5"><h2 className="font-bold">Recent searches</h2><div className="mt-3 flex flex-wrap gap-2">{recent.length?recent.map(item=><button className="btn btn-secondary" onClick={()=>setQuery(item)} key={item}><Search size={14} aria-hidden="true"/>{item}</button>):<p className="muted text-sm">Your recent searches will appear here.</p>}</div></div><div className="panel p-5"><h2 className="font-bold">Quick searches</h2><div className="mt-3 flex flex-wrap gap-2">{shortcuts[user.role].map(item=><button className="btn btn-secondary" onClick={()=>setQuery(item)} key={item}><ArrowRight size={14} aria-hidden="true"/>{item}</button>)}</div></div></section>}
